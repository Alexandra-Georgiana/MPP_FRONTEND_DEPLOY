...
app.post('/api/update', authenticateUser, upload.single('avatar'), async (req, res) => {
  ...
  const email = req.user.email; // Use email from authenticated token
  ...
  try {
    ...
    // Prepare update fields with current values as defaults
    const updateData = {
      favorite_genre: req.body.favoriteGenre || currentUser.favorite_genre,
      favorite_artist: req.body.favoriteArtist || currentUser.favorite_artist,
      bio: req.body.bio || currentUser.bio,
      avatar: req.file ? req.file.filename : (req.body.avatar || currentUser.avatar)
    };

    // Only hash and update password if a new one is provided
    let queryParams = [updateData.favorite_genre, updateData.favorite_artist, updateData.bio, updateData.avatar];
    let passwordClause = '';
    ...
    res.json({ 
      ...
      updatedFields: {
        favoriteGenre: updateData.favorite_genre,
        favoriteArtist: updateData.favorite_artist,
        bio: updateData.bio,
